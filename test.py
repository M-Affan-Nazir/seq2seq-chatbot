...
from tensorflow.compat.v1.keras.layers import Input, Embedding, LSTM, Dense, RepeatVector, Bidirectional, Dropout, TimeDistributed, Concatenate
...
def inference_encoder_model():
    ...
    encoder_model = tf.keras.models.Model(inputs=encoder_inputs, outputs=[enc_output, enc_h, enc_c ])
...
def inference_decoder_model():
    ...
    encoder_outputs_input = Input(shape=(None, 400))  #num_encoder_units = 400. Encoder_output passed here (during inference)
    attention = seq2seq.layers[5]
    attention_output = attention([decoder_outputs,encoder_outputs_input])
    decoder_attention = Concatenate(axis=-1)([decoder_outputs,attention_output])


    dense = seq2seq.layers[7]

    decoder_outputs = dense(decoder_attention)
    decoder_model = tf.keras.models.Model([decoder_input,encoder_outputs_input]+decoder_states_input, [decoder_outputs]+decoder_states)
...
def predict(input):
    ...
    encoder_output, state_h, state_c = encoderModel.predict(input)   #gives h and c states
    ...
    while not stop:
        word_array, h, c = decoderModel.predict([target_sequence, encoder_output]+state)
